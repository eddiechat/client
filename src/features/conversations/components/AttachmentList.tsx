...
import type { AttachmentInfo } from "../../../tauri";
import { formatFileSize, getFileIconType } from "../../../shared";
...
export function AttachmentList({
  ...
  const handleToggle = async () => {
    ...
    if (attachments.length === 0) {
      ...
      setError(null);
      try {
        const result = await getMessageAttachments(folder, uid, account);
            ...
            attachments.map((attachment) => (
              ...
              >
                <div className="w-5 h-5 shrink-0 flex items-center justify-center text-white/60">
                  {getFileIconType(attachment.mime_type) === "image" ? (
                    <svg
                      className="w-4 h-4"
                      viewBox="0 0 24 24"
                      fill="none"
