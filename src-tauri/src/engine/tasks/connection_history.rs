...
pub async fn run_connection_history(
    ...
    let sync_folders = folders::folders_to_sync(&folder_list);

    // Step 2: For each folder, search for messages involving connections
    ...
    for folder_info in &sync_folders {
        ...
        let conn_total = connection_emails.len();
        for (conn_idx, email) in connection_emails.iter().enumerate() {
            ...
            for chunk in new_uids.chunks(200) {
                ...
                let fetch_query = if conn.has_gmail_ext {
                    "(UID FLAGS ENVELOPE BODYSTRUCTURE RFC822.SIZE X-GM-LABELS)"
                } else {
